...
class StarshipAPI: BaseAPI {
    ...
    func fetchStarships(withQuery starshipQuery: String, completion: @escaping (Result<[Starship], APIError>) -> Void) {
        let starshipsUrl = Constants.baseURL.rawValue + Constants.starshipsEndPoint.rawValue
        
        var components = URLComponents(string: starshipsUrl)
        
        let searchQueryItem = URLQueryItem(name: "search", value: starshipQuery)
